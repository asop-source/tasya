...
import { onWillStart, onWillUnmount, useRef, useState, Component } from "@odoo/owl";
import { useService } from "@web/core/utils/hooks";

export class ShBarcodeMobile extends Component {
    static template = "sh_inventory_adjustment_barcode_mobile.BarcodeMobile";

    static props = {
        onBarcodeDetected: { type: Function },
        continuouslyScan: { type: Boolean, optional: true },
    };
    static defaultProps = {
    ...
    setup() {
        ...
        this.detector = null;
        this.stream = null;
        this.deviceId = browser.localStorage.getItem("sh_inventory_adjustment_barcode_mobile_selected_device_id") || null;
    ...
    async onChangeCameraSelection(deviceId) {
        if (deviceId) {
            browser.localStorage.setItem("sh_inventory_adjustment_barcode_mobile_selected_device_id", deviceId);
    ...
    */
    async detectCode() {
        if (this.deviceId && this.videoPreviewRef.el) {
            try {
                if (this.props.continuouslyScan) {
                    await this.formatReader.decodeFromVideoDevice(this.deviceId, this.videoPreviewRef.el, async (result, error) => {
                        if (result && result.text && !error) {
                            ...
                            this.props.onBarcodeDetected(result.text);
                        }
                    });
                } else {
